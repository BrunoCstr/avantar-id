...
// Timeout de segurança: 50 segundos (Next.js API routes têm limite de ~60s)
...
export async function POST(request: NextRequest) {
    ...
    setTimeout(() => {
      resolve(
        NextResponse.json(
          {
            success: false,
            error: "Timeout ao verificar e-mails",
  ...
  const checkEmailsPromise = new Promise<NextResponse>((resolve) => {
    let finished = false;
    ...
    function onSuccess(msg: string) {
      if (!finished) {
        ...
        resolve(
          NextResponse.json({
            success: true,
            message: msg || "Verificação concluída com sucesso",
          })
        );
      }
    }
    ...
    try {
      request.json().then((body) => {
        ...
        if (!companyName || !companyEmail || !companyReceiverEmail || !companyReceiverEmailPassword) {
          return onError(new Error("Parâmetros obrigatórios não fornecidos"));
        }
  ...
  // Race entre timeout e execução
  return Promise.race([timeoutPromise, checkEmailsPromise]);
}

// Função checkEmails refatorada para usar diretamente na API route
                ...
                msg.on("body", function (stream: any) {
                  ...
                  simpleParser(stream)
                    .then(async (parsed: any) => {
                      ...
                      let code: RegExpMatchArray | null = null;
                      
                      // CASO ESPECIAL: admcorretor@qualicorp.net
                      // Procura por padrão específico: "na tela de login: <strong>CODIGO</strong>"
                      if (senderEmail === "admcorretor@qualicorp.net" && html) {
                        console.log("🔍 CASO ESPECIAL: admcorretor@qualicorp.net - Procurando padrão específico no HTML");
                        // Procura por "na tela de login:" seguido de <strong>CODIGO</strong>
                        // Aceita variações com ou sem espaços, com ou sem pontuação após
                        const qualicorpPattern = /na\s+tela\s+de\s+login\s*:\s*<strong>([A-Z0-9]{3,6})<\/strong>/i;
                        const qualicorpMatch = html.match(qualicorpPattern);
                        console.log("  Regex Qualicorp:", qualicorpPattern);
                        console.log("  Match Qualicorp:", qualicorpMatch);
                        if (qualicorpMatch) {
                          code = [qualicorpMatch[1]];
                          console.log("  ✅ Código encontrado via padrão Qualicorp:", qualicorpMatch[1]);
                        } else {
                          console.log("  ⚠️ Padrão Qualicorp não encontrado, tentando métodos padrão");
                        }
                      }
                      
                      // Se não encontrou no caso especial, continua com os métodos padrão
                      if (!code) {
                        // Primeiro tenta capturar códigos com hífen e espaços (ex: 020 - 055)
                        code = text?.match(/\b\d{3}\s*-\s*\d{3}\b/);
                      }
                      ...
                      if (!code && html) {
                        ...
                        // CASO ESPECIAL: admcorretor@qualicorp.net - verificar antes de limpar HTML
                        if (senderEmail === "admcorretor@qualicorp.net" && !code) {
                          console.log("🔍 CASO ESPECIAL: admcorretor@qualicorp.net - Verificando HTML original antes de limpar");
                          const qualicorpPattern = /na\s+tela\s+de\s+login\s*:\s*<strong>([A-Z0-9]{3,6})<\/strong>/i;
                          const qualicorpMatch = html.match(qualicorpPattern);
                          if (qualicorpMatch) {
                            code = [qualicorpMatch[1]];
                            console.log("  ✅ Código encontrado via padrão Qualicorp no HTML original:", qualicorpMatch[1]);
                          }
                        }
                        ...
                        // Se ainda não encontrou, continua com métodos padrão no HTML limpo
                        if (!code) {
                      ...
                      if (code) {
                        ...
                        try {
                          ...
                          if (!company.code || company.code !== code[0]) {
                            try {
                              // Garantir que admin está inicializado
                              if (admin.apps.length === 0) {
                                // Se não estiver inicializado, getAdminDb() já inicializa
                                await getAdminDb();
                              }
